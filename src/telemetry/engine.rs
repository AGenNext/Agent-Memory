...
pub type AnalyserFn = Arc<
    dyn Fn(
        ...
        Arc<Store>,
        Arc<Config>,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<TelemetryResult>> + Send>>
    + Send + Sync,
>;
...
impl TelemetryEngine {
    ...
    fn list_available(&self, agent_id: &str, window_days: i64) -> TelemetryResult {
    ...
    fn unknown_query(&self, agent_id: &str, query: &str, window_days: i64) -> TelemetryResult {
    ...
    async fn run_all(&self, agent_id: &str, window_days: i64) -> Result<TelemetryResult> {
...
fn analyse_decay_tuning(
    ...
    Box::pin(async move {
        ...
        Ok(TelemetryResult { query: "decay_tuning".to_string(), agent_id, window_days, findings, recommendations, config_suggestions })
    })
}
...
fn analyse_recall_health(
    ...
    Box::pin(async move {
        ...
        Ok(TelemetryResult { query: "recall_health".to_string(), agent_id, window_days, findings, recommendations, config_suggestions: BTreeMap::new() })
    })
}
...
fn analyse_conflict_patterns(
    ...
    Box::pin(async move {
        ...
        Ok(TelemetryResult { query: "conflict_patterns".to_string(), agent_id, window_days, findings, recommendations: vec![], config_suggestions: BTreeMap::new() })
    })
}
...
fn analyse_memory_growth(
    ...
    Box::pin(async move {
        ...
        Ok(TelemetryResult { query: "memory_growth".to_string(), agent_id, window_days, findings, recommendations: vec![], config_suggestions: BTreeMap::new() })
    })
}
...
fn analyse_reinforcement(
    ...
    Box::pin(async move {
        ...
        Ok(TelemetryResult { query: "reinforcement".to_string(), agent_id, window_days, findings, recommendations, config_suggestions })
    })
}
...
fn analyse_session_patterns(
    ...
    Box::pin(async move {
        ...
        Ok(TelemetryResult { query: "session_patterns".to_string(), agent_id, window_days, findings, recommendations: vec![], config_suggestions: BTreeMap::new() })
    })
}
